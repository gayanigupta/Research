...
void findChordalEdgesWithEliminationOrder(int *graph, int numVertices, int *eliminationOrder, int numEdges)
{
    ...
    for (int i = 0; i < numVertices; i++)
    {
        ...
        if (minNeghbor < i)
        {
            ...
            // If the neighbors of the lowest parent are a subset of the current node's neighbors,
            // mark all edges between the current node and its lowest parent as chordal.
