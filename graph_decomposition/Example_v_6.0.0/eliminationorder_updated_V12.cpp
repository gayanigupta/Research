...
vector<Edge> primMaxSpanningTree(Graph& graph,  vector<int>& eliminationOrder) {
    int n = graph.V;
    vector<Edge> maxSpanningTree;
    vector<bool> visited(n + 1, false);
    priority_queue<Edge> pq; // Edges with negative weights to reverse priority

    int startNode = eliminationOrder[0]; // Start from the first node in elimination order
    for (int v : graph.adj[startNode]) {
        pq.push(Edge(startNode, v, -1)); // Negative weight to reverse priority
    }
    ...
    while (!pq.empty()) {
        ...
        int u = e.node1;
        int v = e.node2;
        int weight = e.edge_wt;

        if (visited[v]) continue;
        visited[v] = true;

        maxSpanningTree.push_back(Edge(u, v, -weight)); // Reverse weight back to positive

        for (int w : graph.adj[v]) {
            if (!visited[w]) {
                pq.push(Edge(v, w, -1)); // Negative weight to reverse priority
            }
        }
    }
...
print_network(x1);
   ...
   ifstream dataFile(argv[3], ios::in); // Open input file
    ...
    vector<Edge> maxSpanningTree = primMaxSpanningTree(chordalGraph, eliminationOrder);

    cout << "Edges of the maximum spanning tree:" << endl;
