...
void runEliminationOrdering(A_Network &src, vector<int> &orderList, vector<vector<int> > &tree, DEGREE_MODE degreeMode)
{
	...
	// Process till all vertices of the graph are processed.
	while (orderList.size() != count)
	{
		// Store degrees of all unprocessed vertices
		vector<pair<int, float>> verticesWithDegree;
		for (int i = 0; i < count; i++)
		{
			if (processed[i] || x[i].ListW.size() == 0)
				continue;

			pair<int, float> item;
			item.first = i;
			// Get the degree value according to degree mode.
			item.second = (degreeMode == PURE_DEGREE) ? x[i].ListW.size() : calculateLocalDegreeCoefficient(x, i);

			verticesWithDegree.push_back(item);
		}

		if (verticesWithDegree.size() == 0)
			break;

		// Sort according to degree of vertex.
		heapSort(verticesWithDegree, verticesWithDegree.size());

		int sel_vertex = verticesWithDegree[0].first;
		...
		for (int i = 0; i < listW.size(); ++i)
		{
			...
			if (removeUndirectedEdge(x, sel_vertex, neighbor))
			{
